...
| 功能 | 详细说明 |
|------|----------|
| **动态模型选择** | 支持 24+ 个 AI 模型（DeepSeek、Qwen、OpenAI 等），5 级质量分类 |
...
**功能亮点**：
- 📊 24+ AI 模型支持（DeepSeek、Qwen、OpenAI、Kimi 等）
...
CREATE TABLE ai_model_config (
  ...
  provider VARCHAR(50),          // 提供商（deepseek/qwen/openai）
...
**后端**：
- 新增 `DynamicAiModelFactory` - 动态模型工厂
- 新增 `AiPlanningService` - 计划生成服务
- 新增 `CodeValidationTool` - 代码验证工具
...
3. **AI 推理时间线性增加**：
   - DeepSeek Reasoner 模型处理长上下文时推理速度显著下降
...
- **Claude Code** - Anthropic 官方 CLI，用于代码生成和重构
- **Chrome DevTools MCP** - 浏览器调试与自动化
- **Firecrawl MCP** - 网页内容抓取和分析
- **IDE MCP** - IDE 集成开发环境支持
