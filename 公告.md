...
graph LR
    ...
    B1 --> C1[GPT-4<br/>固定 API Key<br/>固定参数]
...
graph TB
    ...
    H2 -->|SIMPLE| J1[基础模型<br/>gpt-5-low<br/>qwen-turbo<br/>1.0-1.5x 质量系数]
    H2 -->|MEDIUM| J2[标准模型<br/>qwen-plus<br/>gpt-5-medium<br/>1.5-2.0x 质量系数]
    H2 -->|HARD| J3[高级模型<br/>gpt-5-codex<br/>deepseek-v3<br/>2.0-2.5x 质量系数]
    H2 -->|EXPERT| J4[专家模型<br/>o1-reasoner<br/>deepseek-r1<br/>2.5-3.5x 质量系数]
...
| **v1.1.0** | 2025-11-15 | • AI 模型动态选择（20+ 模型，4 级分类）<br>• 两阶段代码生成（节省 20-30% Token）<br>• 代码自动验证（ESLint 集成）<br>• 积分系统完善（修复 8 个缺陷）<br>• 企业级部署（一键部署 + 备份回滚）<br>• 前端模块化重构（composables 架构）<br>• 完整监控体系（Prometheus 指标） | ✅ 已发布 |
| **v1.2.0** | 2026 Q1 | • 轻量级数据存储（零门槛数据管理）<br>• VUE 模板商店（预置模板快速克隆） | 🔄 规划中 |
